...
class HFBState:
    ...
    def canonical_error(self):
        m = len(self.U)
        return max(np.linalg.norm(self.U.conj().T @ self.U +
                                  self.V.conj().T @ self.V - np.eye(m)),
...
class HFBHamiltonian:
    def __init__(self, h, interaction):
        ...
        for a, b in [(self.h, self.h.conj().T),
    ...
    def energy(self, state):
        ...
        e = (np.einsum('ij,ji->', self.h, r)
             + .5 * np.einsum('ijkl,ki,lj->', self.v, r, r)
             + .25 * np.einsum('ijkl,ij,kl->', self.v, k.conj(), k))
        if abs(e.imag) > 1e-8:
...
def solve_hfb(hamiltonian, neutron_modes, targets, *, starts=3, seed=0,
              maxiter=300, tolerance=1e-8, initial_parameters=None):
    ...
    if (targets.shape != (2,) or not np.isfinite(targets).all()
            or np.any(targets < 0) or np.any(targets > capacities)):
    ...
    for attempt in range(starts):
        x = (np.array(initial_parameters, float) if attempt == 0 and
             initial_parameters is not None else
             rng.normal(scale=.5 / np.sqrt(m), size=m * (m - 1)))
        fit = minimize(objective, x, method='SLSQP',
        ...
        attempts.append({'converged': ok, 'energy': float(fit.fun),
                         'number_error': residual, 'message': str(fit.message)})
        candidates.append((ok, residual, fit, state))
    feasible = [c for c in candidates if c[0]]
    best = (min(feasible, key=lambda c: c[2].fun) if feasible else
    ...
    gradient = np.array([(objective(fit.x+d)-objective(fit.x-d))/(2*step)
                         for d in directions])
    jacobian = np.array([(constraint(fit.x+d)-constraint(fit.x-d))/(2*step)
    ...
    return HFBResult(state, float(fit.fun), numbers(state), ok,
                     str(fit.message), fit.x, attempts, multipliers, stationarity)
